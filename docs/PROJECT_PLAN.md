...
AI 协同个人知识库管理系统是一个**桌面应用程序**，帮助用户：
- 管理个人笔记和文档
- 基于私有知识库进行智能问答
- 利用多 Agent 协作完成深度研究任务
- 可视化查看个人知识结构
...
| 用户痛点 | 解决方案 |
|----------|---------|
| 笔记散落各处，找不到 | 统一管理 + AI 语义搜索 |
| 看了很多文档，记不住 | RAG 知识库问答 |
| 写报告费时间 | 多 Agent 自动研究 + 写作 |
| 不知道自己学了什么 | 知识图谱可视化 |
...
| 阶段 | 名称 | 核心内容 | 预计时间 | 产出 |
|------|------|---------|---------|------|
| **P1** | 基础通信 | FastAPI + LLM 对话 + 流式 + 桌面壳 | 3-4天 | 能聊天的桌面应用 |
| **P2** | 笔记系统 | CRUD + SQLite + 标签 + 富文本编辑 | 3-4天 | 完整的笔记管理 |
| **P3** | RAG 引擎 | 文档上传 → 切片 → 向量化 → 检索问答 | 4-5天 | 知识库问答 |
| **P4** | 工具调用 | Function Calling + ReAct Agent | 3-4天 | Agent 自主用工具 |
| **P5** | 多Agent | LangGraph 编排 + 4 Agent 协作 | 4-5天 | 深度研究系统 |
| **P6** | UI 打磨 | 4页面完善 + 样式 + 交互优化 | 3-4天 | 精美桌面应用 |
| **P7** | 知识图谱 | ECharts 可视化 + 统计看板 | 2-3天 | 数据可视化 |
| **P8** | 打包发布 | PyInstaller + 测试 + README + 简历 | 2-3天 | .exe + GitHub |
