...
class ReviewService:
    ...
    def get_calendar(self, db: Session, notebook_id: int, year: int, month: int) -> dict:
        """返回指定月份的复习热力图数据"""
        ...
        logs = (
            db.query(ReviewLog)
            .filter(
                ReviewLog.created_at >= first_day,
                ReviewLog.created_at < next_month,
            )
            .all()
        )

        from app.models.note import Note
        note_ids = set(log.note_id for log in logs)
        notes = db.query(Note).filter(Note.id.in_(note_ids)).all()
        valid_note_ids = {n.id for n in notes if n.notebook_id == notebook_id}

        day_stats: dict[str, dict] = defaultdict(
            lambda: {"count": 0, "total_correct": 0, "total_q": 0}
        )
        for log in logs:
            if log.note_id not in valid_note_ids:
                continue
            day = log.created_at.strftime("%Y-%m-%d")
            day_stats[day]["count"] += 1
            day_stats[day]["total_correct"] += log.correct_count
            day_stats[day]["total_q"] += log.total_count

        days = []
        total_reviews = 0
        total_questions = 0
        for day_str, stats in sorted(day_stats.items()):
            avg_score = (
                round(stats["total_correct"] / stats["total_q"] * 100, 1)
                if stats["total_q"] else 0
            )
            days.append({
                "date": day_str,
                "count": stats["count"],
    ...
    def get_day_detail(self, db: Session, notebook_id: int, date_str: str) -> dict:
        """返回某一天的复习详情"""
        try:
        ...
        logs = (
            db.query(ReviewLog)
            .filter(
                ReviewLog.created_at >= target_date,
                ReviewLog.created_at < next_date,
            )
            .all()
        )
        ...
        note_ids = set(log.note_id for log in logs)
        notes = db.query(Note).filter(Note.id.in_(note_ids)).all()
        note_map = {n.id: n for n in notes if n.notebook_id == notebook_id}

        cluster_notes = db.query(ClusterNote).filter(
            ClusterNote.note_id.in_(note_ids)
        ).all()
        cn_map: dict[int, int] = {}
        for cn in cluster_notes:
            if cn.note_id not in cn_map:
                cn_map[cn.note_id] = cn.cluster_id

        cluster_ids = set(cn_map.values())
        clusters = db.query(ConceptCluster).filter(
            ConceptCluster.id.in_(cluster_ids)
        ).all()
        ...
        correct_q = 0
        for log in logs:
            note = note_map.get(log.note_id)
            if not note:
                continue
            cid = cn_map.get(log.note_id)
            reviews.append({
                "note_id": log.note_id,
                "note_title": note.title or "无标题",
                "cluster_name": cluster_map.get(cid, "未归类") if cid else "未归类",
                "correct": f"{log.correct_count}/{log.total_count}",
                "rating": log.rating or "—",
            })
            total_q += log.total_count
            correct_q += log.correct_count
        ...
        return {
            ...
            "score": round(correct_q / total_q * 100, 1) if total_q else 0,
        }
