...
| 项目 | 值 |
|------|-----|
| 已完成阶段 | P1-P7 全部完成 (55/55) |
| 当前阶段 | **S1 — 知识进阶**（Agent 对话评估掌握度） |
| 当前任务 | 1.0 数据模型 + 1.1 MasteryAgent + 1.2 API |
| 后续 | S2 温故知新（间隔复习，复用 P6 出题）/ S3 知识结构（概念地图，复用 P7 图谱） |
| 页面精简 | 6 页: 智能笔记/知识问答/温故知新/知识进阶/知识结构/回收站（删数据看板，考 quiz→温故知新，图谱→知识结构） |
| GitHub | https://github.com/Mcloseyb/ai-second-brain |
| 上次会话 | 2026-08-03，重规划学习体系（S1/S2/S3 三板块），开始 S1 知识进阶 |
...
1. **启动后端**: `cd H:\agent && .venv\Scripts\activate && uvicorn app.main:app --app-dir backend --host 127.0.0.1 --port 8000`
2. **启动桌面**: `python desktop\main.py --no-backend`（后端先启动则用此方式）
3. **或直接**: `python desktop\main.py`（自动启动后端，但可能不稳定）
4. **前端开发**: `cd frontend && npm run dev`（Vite dev server，端口 5173）
5. **读取** `docs/TODO.md` — P8 第一个未勾选的任务就是当前任务
